...
function CapacityWidget({ pct }: { pct: number }) {
  ...
  const theme =
    clamped < 50  ? { wave: "#22d3ee", wave2: "#06b6d4", text: "text-cyan-300",  glow: "rgba(34,211,238,0.55)" } :
    clamped <= 80 ? { wave: "#fbbf24", wave2: "#f97316", text: "text-amber-300", glow: "rgba(251,191,36,0.55)" } :
                   { wave: "#fb7185", wave2: "#f43f5e", text: "text-rose-300",  glow: "rgba(244,63,94,0.55)"  };
  ...
  return (
    <div className="flex flex-col gap-1.5 px-4 py-3.5 rounded-2xl bg-slate-900/60 border border-white/7 items-center justify-center">
      <div className="flex items-center gap-1.5 text-slate-500 text-[10px] font-plus-jakarta uppercase tracking-widest self-start">Fill Level</div>
      <div className="relative size-16 shrink-0">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <p className={`font-outfit font-extrabold text-xl leading-none ${theme.text}`}
    ...
    <div className="min-h-full flex flex-col gap-5 max-w-6xl mx-auto">
      ...
      <div className="flex items-start justify-between gap-4 pt-1">
        ...
        <div className="flex items-center gap-2" data-print-hide="true">
          ...
          <motion.button onClick={handlePrint} disabled={isPrinting}
            ...
            style={{
              background: isPrinting ? "rgba(139,92,246,0.25)" : "linear-gradient(135deg, rgba(139,92,246,0.18) 0%, rgba(168,85,247,0.1) 100%)",
      ...
      {loading
        ...
        : <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
            ...
            <SensorWidget icon={<Thermometer size={11} />} label="Temperature" value={silo?.temperature ?? 0} unit="°C" gradient="bg-gradient-to-r from-amber-400 to-orange-600" glow="rgba(251,146,60,0.55)" />
            <SensorWidget icon={<Droplets size={11} />}    label="Humidity"    value={silo?.humidity ?? 0}    unit="%" gradient="bg-gradient-to-r from-cyan-400 to-blue-500"  glow="rgba(34,211,238,0.55)" />
      ...
      <section data-print-section="avoid-break">
        <div className="rounded-2xl bg-slate-900/60 border border-white/6 p-5">
          {loading
