...
// ─── Nav items ────────────────────────────────────────────────────────────────
// Each item is either a live route or a "coming soon" / settings action.

const NAV: Array<
  | { type: "link";    href: string; label: string; icon: React.ReactNode; match: (p: string) => boolean }
  | { type: "coming"; label: string; icon: React.ReactNode; name: string }
  | { type: "settings"; label: string; icon: React.ReactNode }
> = [
  {
    type: "link",
    href: "/",
    ...
    // Active only on exact root — not on /silos/[id]
    match: (p) => p === "/",
  },
  {
    type: "link",
    ...
    label: "Silos",
    icon: <Wheat size={16} />,
    // Will become /silos once that route exists; for now share the root.
    // Active when on root OR a silo detail page.
    match: (p) => p.startsWith("/silos") || (p === "/" ? false : false),
    // ↑ Currently never active so it won't clash with Dashboard.
    // When /silos route is created, change to: (p) => p === "/silos"
  },
  { type: "coming",  label: "Live Map", icon: <Map      size={16} />, name: "Live Map"             },
  { type: "coming",  label: "Reports",  icon: <BarChart2 size={16} />, name: "Analytics & Reports" },
  { type: "settings",label: "Settings", icon: <Settings  size={16} />                              },
];
...
function ComingSoonToast({ toast, onDismiss }: { toast: Toast; onDismiss: () => void }) {
  return (
    <motion.div
      ...
      className="
        flex items-start gap-3 px-4 py-3 rounded-2xl w-64
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="font-plus-jakarta text-slate-500 text-xs mt-0.5">Coming soon — stay tuned!</p>
      </div>
      <button onClick={onDismiss} className="text-slate-600 hover:text-slate-300 transition-colors" aria-label="Dismiss">
        <X size={13} />
      </button>
    </motion.div>
  );
    ...
    <>
      ...
      ">
        ...
        <nav className="flex-1 px-3 space-y-0.5">
          {NAV.map((item, i) => {
            if (item.type === "link") {
              // Precise active check — each nav item provides its own matcher
            ...
            if (item.type === "settings") {
              return (
                <button
                  ...
                  `}
                >
                  <span className={`${settingsOpen ? "text-emerald-400" : "text-slate-600 group-hover:text-slate-400"} transition-colors`}>
                    {item.icon}
                  </span>
                  {item.label}
                </button>
            ...
            return (
              <button
                key={i}
                onClick={() => addToast(item.name)}
                className="
                  group w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left
                  text-sm font-medium text-slate-600
                  hover:text-slate-400 hover:bg-white/[0.03] transition-all duration-150
                "
              >
        ...
        <div className="px-5 py-4 mt-auto">
          <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl bg-white/[0.03] border border-white/[0.05]">
            <div className="flex-1">
              <p className="font-plus-jakarta text-[10px] text-slate-500">v0.1.0 — Beta</p>
            </div>
