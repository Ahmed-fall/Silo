...
function CustomTooltip({
  ...
  // Determine if this is a forecast point from the dataKey name
  const isForecast = payload.some((p) => p.name.includes("forecast"));

  return (
    <div className="rounded-2xl border border-white/[0.08] bg-slate-950/95 backdrop-blur-xl px-4 py-3 shadow-2xl shadow-black/70 text-sm min-w-45">

      {/* Header: time + Historical/Forecast tag */}
      <div className="flex items-center justify-between gap-3 mb-2.5">
        <div>
          <p className="font-outfit text-slate-500 text-[10px] tracking-[0.18em] uppercase">{time}</p>
          <p className="font-plus-jakarta text-slate-600 text-[9px]">{dayLabel}</p>
        </div>
        {isForecast ? (
          <span
            ...
            style={{
              ...
              color: "#e879f9",
              boxShadow: "0 0 10px rgba(232,121,249,0.25)",
            }}
          >
            🔮 AI Prediction
          </span>
        ) : (
          <span className="flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[9px] font-semibold font-outfit tracking-wide bg-slate-800/80 border border-slate-700/50 text-slate-400">
            📊 Historical
          </span>
        )}
      </div>
      ...
      {payload.map((entry) => {
        ...
        return (
          <div key={entry.name} className="flex items-center gap-2 mb-1.5 last:mb-0">
            ...
            />
            <span className="font-plus-jakarta text-slate-300 text-xs flex-1">{label}</span>
      ...
      {/* AI disclaimer */}
      {isForecast && (
        <p className="font-plus-jakarta text-[9px] text-slate-600 mt-2 pt-2 border-t border-white/5">
...
export default function SensorChart({ data }: { data: SensorReading[] }) {
  ...
  const chartData: ChartPoint[] = useMemo(() => {
    const forecast = generateForecast(data, 12);
